...
class AiohttpFactory:
    ...
    async def start(self) -> None:
        ...
        if not self.session or self.session.closed:
            ...
            connector = aiohttp.TCPConnector(
                limit_per_host=self.limit_per_host, limit=self.limit
            )
    ...
    async def fetch_data(self, method: str, url: str, content_key: str, **kwargs):
        ...
        while attempt <= self.max_retries:
            try:
                if not self.session or self.session.closed:
                    raise RuntimeError(
                        "Aiohttp session is not initialized. Call .start() first."
                    )
    ...
    async def post(
        self, url, body=None, content_key="content", **kwargs
    ) -> dict[str, Any]:
        headers: dict[str, str] = kwargs.pop("headers", {})
        headers.setdefault("Content-Type", "application/json")
        return await self.fetch_data(
            "POST", url, content_key, json=body, headers=headers, **kwargs
        )
    ...
    async def put(
        ...
        headers.setdefault("Content-Type", "application/json")
        return await self.fetch_data(
            "PUT", url, content_key, json=body, headers=headers, **kwargs
        )
    ...
    async def patch(
        ...
        headers.setdefault("Content-Type", "application/json")
        return await self.fetch_data(
            "PATCH", url, content_key, json=body, headers=headers, **kwargs
        )
